# enc 3 dec 2 single-stage interpretable with fixed knee
# noiseless feedback
import numpy as np
import torch
...
class AE(torch.nn.Module): 
    def __init__(self, args):
        ...
        self.d6 = torch.nn.Parameter(torch.rand(4), requires_grad = True)
        self.d7 = torch.nn.Parameter(torch.rand(4), requires_grad = True)
    ...
    def forward(self, information_bits, forward_noise, feedback_noise):
        ...
        r1 = torch.tanh(self.d1[0] * rec_info_current - self.d1[1] * rec_parity_substraction_current - self.d1[2] * rec_parity_sum_future + self.d1[3])
        r2 = torch.tanh(self.d2[0] * rec_info_current - self.d2[1] * rec_parity_substraction_current - self.d2[2] * rec_parity_sum_future + self.d2[3])
        r3 = torch.tanh(self.d3[0] * rec_info_current - self.d3[1] * rec_parity_substraction_current - self.d3[2] * rec_parity_sum_future + self.d3[3])
        r4 = torch.tanh(self.d4[0] * rec_info_current - self.d4[1] * rec_parity_substraction_current - self.d4[2] * rec_parity_sum_future + self.d4[3])
        r5 = torch.tanh(self.d5[0] * rec_info_current - self.d5[1] * rec_parity_substraction_current - self.d5[2] * rec_parity_sum_future + self.d5[3])
        r6 = torch.tanh(self.d6[0] * rec_info_current - self.d6[1] * rec_parity_substraction_current - self.d6[2] * rec_parity_sum_future + self.d6[3])
        r7 = torch.tanh(self.d7[0] * rec_info_current - self.d7[1] * rec_parity_substraction_current - self.d7[2] * rec_parity_sum_future + self.d7[3])
...
args.initial_weights = 'logs/noiseless/single/enc3dec2_single.pt'
