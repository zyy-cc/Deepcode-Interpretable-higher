# enc 3 dec 4 single-stage interpretable with fixed knee
# noiseless feedback
import numpy as np
import torch
...
class AE(torch.nn.Module): 
    def __init__(self, args):
        ...
        # Decoder
        self.d1 = torch.nn.Parameter(torch.rand(6), requires_grad = True)
        self.d2 = torch.nn.Parameter(torch.rand(6), requires_grad = True)
        self.d3 = torch.nn.Parameter(torch.rand(6), requires_grad = True)
        self.d4 = torch.nn.Parameter(torch.rand(6), requires_grad = True)
        self.d5 = torch.nn.Parameter(torch.rand(6), requires_grad = True)
        self.d6 = torch.nn.Parameter(torch.rand(6), requires_grad = True)
        self.d7 = torch.nn.Parameter(torch.rand(6), requires_grad = True)
        self.l = torch.nn.Parameter(torch.rand(7), requires_grad = True)
    ...
    def forward(self, information_bits, forward_noise, feedback_noise):
        ...
        for idx_bit in range(information_bits.shape[1]):
            ...
            if idx_bit == 0:
                ...
                vh4 = torch.ones((num_samples_input,1))
                vh5 = -1 * torch.ones((num_samples_input,1))

                vh6 = torch.ones((num_samples_input,1))
                vh7 = torch.ones((num_samples_input,1))
                all_h4.append(vh4)
                all_h5.append(vh5)
                all_h6.append(vh6)
                all_h7.append(vh7)
        ...
        rec_info_current = noisy_codewords[:,:,0].view(num_samples_input, block_len, 1)
        rec_parity_substraction_current = (noisy_codewords[:,:,1] - noisy_codewords[:,:,2]).view(num_samples_input, block_len, 1)

        # future parity bits
        parity_one_time = noisy_codewords[:,1:,1:]
        parity_one_time_step = torch.cat((parity_one_time, torch.zeros(num_samples_input, 1, 2)), dim=1)
        rec_parity_sum_future = (parity_one_time_step[:,:,0] + parity_one_time_step[:,:,1]).view(num_samples_input, block_len, 1)

        # future future parity bits
        parity_two_time = noisy_codewords[:,2:,1:]
        parity_two_time_step = torch.cat((parity_two_time, torch.zeros(num_samples_input, 2, 2)), dim=1)
        rec_parity_sum_future2 = (parity_two_time_step[:,:,0] + parity_two_time_step[:,:,1]).view(num_samples_input, block_len, 1)


        # 3 time step future future  parity bits
        parity_three_time = noisy_codewords[:,3:,1:]
        parity_three_time_step = torch.cat((parity_three_time, torch.zeros(num_samples_input, 3, 2)), dim=1)
        rec_parity_sum_future3 = (parity_three_time_step[:,:,0] + parity_three_time_step[:,:,1]).view(num_samples_input, block_len, 1)
        
        
        r1 = torch.tanh(self.d1[0] * rec_info_current - self.d1[1] * rec_parity_substraction_current - self.d1[2] * rec_parity_sum_future - self.d1[3] * rec_parity_sum_future2 - self.d1[4] * rec_parity_sum_future3 + self.d1[5])
        r2 = torch.tanh(self.d2[0] * rec_info_current - self.d2[1] * rec_parity_substraction_current - self.d2[2] * rec_parity_sum_future - self.d2[3] * rec_parity_sum_future2 - self.d2[4] * rec_parity_sum_future3 + self.d2[5])
        r3 = torch.tanh(self.d3[0] * rec_info_current - self.d3[1] * rec_parity_substraction_current - self.d3[2] * rec_parity_sum_future - self.d3[3] * rec_parity_sum_future2 - self.d3[4] * rec_parity_sum_future3 + self.d3[5])
        r4 = torch.tanh(self.d4[0] * rec_info_current - self.d4[1] * rec_parity_substraction_current - self.d4[2] * rec_parity_sum_future - self.d4[3] * rec_parity_sum_future2 - self.d4[4] * rec_parity_sum_future3 + self.d4[5])
        r5 = torch.tanh(self.d5[0] * rec_info_current - self.d5[1] * rec_parity_substraction_current - self.d5[2] * rec_parity_sum_future - self.d5[3] * rec_parity_sum_future2 - self.d5[4] * rec_parity_sum_future3 + self.d5[5])
        r6 = torch.tanh(self.d6[0] * rec_info_current - self.d6[1] * rec_parity_substraction_current - self.d6[2] * rec_parity_sum_future - self.d6[3] * rec_parity_sum_future2 - self.d6[4] * rec_parity_sum_future3 + self.d6[5])
        r7 = torch.tanh(self.d7[0] * rec_info_current - self.d7[1] * rec_parity_substraction_current - self.d7[2] * rec_parity_sum_future - self.d7[3] * rec_parity_sum_future2 - self.d7[4] * rec_parity_sum_future3 + self.d7[5])

        decoder_output = self.l[0] * r1 + self.l[1] * r2 + self.l[2] * r3 + self.l[3] * r4 + self.l[4] * r5 + self.l[5] * r6 + self.l[6] * r7 
            
        output     = torch.sigmoid(decoder_output)

        all_hidden_states = []
        return all_hidden_states, codewords, output
...
args.initial_weights = 'logs/noiseless/single/enc3dec4_single.pt'
