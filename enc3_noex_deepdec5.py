# enc 3 without entanglement + deep decoder with 5 hidden states
# noiseless feedback
import numpy as np
import torch
...
class AE(torch.nn.Module): 
    def __init__(self, args):
        ...
        self.m = torch.nn.Parameter(torch.rand(4), requires_grad = True)
    ...
    def forward(self, information_bits, forward_noise, feedback_noise):
        ...
        def h6(n2, n3, h4):
            ...
            h7_{i-1}
            """
            res = torch.tanh(self.m[0] * n2 + self.m[1] * n3 + self.m[2] * h4  + self.m[3]) # + self.m[3] * h6
            return res
        ...
        def h7(n2, n3, h5):
            ...
            h6_{i-1}
            """
            res = torch.tanh(- self.m[0] * n2 - self.m[1] * n3 - self.m[2] * h5  + self.m[3]) # + self.m[3] * h7
        ...
        for idx_bit in range(information_bits.shape[1]):
            ...
            if idx_bit == 0:
                ...
                vh6 = h6(input_tmp[:,:,2], input_tmp[:,:,3], all_h4[idx_bit-1])
                vh7 = h7(input_tmp[:,:,2], input_tmp[:,:,3], all_h5[idx_bit-1])
...
args.initial_weights = 'logs/noiseless/enc3_noex_deepdec5_snr0.pt'
